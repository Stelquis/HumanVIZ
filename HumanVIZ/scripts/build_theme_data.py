#!/usr/bin/env python3
"""
Build theme analysis and frontend visualization data for Task 3.
Reads p3_themes.json, computes: type-theme coverage matrix, chi-square
independence tests (scipy), distinctive theme ranking per genre, co-occurrence
with PMI/NPMI scoring, theme combo patterns, archetypes, hierarchical clustering,
source-era theme evolution, genre cosine similarity, extraction examples,
and theme richness distribution.

Input:  data/processed/p3_themes.json
Output: src/data/theme-data.json
"""
import json
import math
...
P3_THEMES = ROOT / "data" / "processed" / "p3_themes.json"
OUTPUT = ROOT / "src" / "data" / "theme-data.json"
...
# Representative thematic patterns from co-occurrence and genre affinity
archetypes = [
    {
        "id": "court_power",
        "name": "宫廷权谋型",
...
# ── 9. Hierarchical clustering of themes (UPGMA) ─────────────────────
# Uses co-occurrence frequency to compute theme similarity, then
# performs Unweighted Pair Group Method with Arithmetic Mean clustering.
...
# ── 10. Chi-square independence test ──────────────────────────────────
# Tests whether each theme's distribution across genre types is independent.
# Uses scipy.stats.chi2_contingency for proper p-value computation.
chi_square = {}
for theme in THEME_ORDER:
    observed = []
    for genre in TYPE_ORDER:
        genre_scripts = [s for s in scripts if s["genre"] == genre]
        has_theme = sum(1 for s in genre_scripts if s["theme_present"].get(theme, 0) == 1)
    ...
    if observed.sum() > 0 and observed.min() >= 0:
        ...
        chi_square[theme] = {"chi2": 0, "p": 1.0, "dof": len(TYPE_ORDER) - 1}
...
print(f"Task 3 theme analysis & frontend data written to {OUTPUT}")
print(f"  {N} scripts, {len(combo_counter)} unique theme combinations across {len(TYPE_ORDER)} genre types")
print(f"  {len(archetypes)} archetypes, {len(top_combos)} top combos")
print(f"  {len(pmi_scores)} co-occurrence pairs with PMI/NPMI scores")
print(f"  chi-square tests: {len(chi_square)} themes, dof={chi_square.get(THEME_ORDER[0], {}).get('dof', 'N/A')}")
