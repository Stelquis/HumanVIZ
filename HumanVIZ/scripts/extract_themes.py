"""
Extract 12-dimension theme vectors from 1473 Peking opera play plot summaries.

Uses weighted keyword matching against a domain-informed theme taxonomy:
  score = Σ(count(keyword) × len(keyword))
Longer keywords receive higher weight (more precise matches).
Scores are normalized to [0,1]; presence is binary (score > 0).

Input:  Raw JSON play files in data/raw/dataSet/
        Genre mapping from data/processed/db_exports/剧目类型.json
Output: data/processed/p3_themes.json — theme vectors, normalized scores,
        presence flags, matched keywords, type-theme stats, co-occurrence pairs
"""
...
BASE_DIR = ROOT / "data" / "raw" / "dataSet"
GENRE_PATH = ROOT / "data" / "processed" / "db_exports" / "剧目类型.json"
OUTPUT_PATH = ROOT / "data" / "processed" / "p3_themes.json"
...
def main():
    print("=" * 60)
    print("Theme Extraction — Task 3")
    print("=" * 60)

    # Load genre type mapping
    genre_map = {}
    ...
    # Collect all JSON play files from raw dataset
    all_jsons = []
    for folder_path in sorted(BASE_DIR.iterdir()):
        if folder_path.is_dir():
            for fpath in folder_path.glob("*.json"):
                all_jsons.append(fpath)

    print(f"Found {len(all_jsons)} JSON play files")
    ...
    for jpath in tqdm(all_jsons, desc="Extracting themes", unit="play"):
        try:
    ...
    print(f"\nDone! Successful: {len(results)}, Failed: {len(errors)}")
    print(f"Output: {OUTPUT_PATH}")

    # Summary
    print("\n===== Theme Distribution Summary =====")
    for t, stats in sorted(theme_overall.items(), key=lambda x: x[1]['script_count'], reverse=True):
        print(f"  {t}: {stats['script_count']} plays ({stats['pct']}%), avg_score={stats['avg_score']}")

    print(f"\nAverage themes per play: {np.mean([r['active_theme_count'] for r in results]):.1f}")
