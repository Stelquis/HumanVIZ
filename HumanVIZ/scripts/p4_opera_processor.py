#!/usr/bin/env python3
"""
p4_opera_processor.py — 京剧剧本 → Story Ribbon 格式转换器

最大程度复用 HumanVIZ 现有故事丝带（Story Ribbon）可视化管线的数据结构，
将京剧原始 JSON 转换为 gatsby-new.json 兼容格式。

用法:
    python p4_opera_processor.py                          # 默认处理代表性样本
    python p4_opera_processor.py --input <file.json>      # 处理单个剧本
    python p4_opera_processor.py --samples                # 仅输出代表性样本
    python p4_opera_processor.py --all                    # 批量处理全部
    python p4_opera_processor.py --list-samples           # 列出代表性样本
"""
