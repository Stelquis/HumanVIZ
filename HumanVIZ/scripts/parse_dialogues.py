...
用法:
    ...
    # 解析单个剧本（从 JSON 文件）
    python scripts/parse_dialogues.py parse --json-file data/dataSet/01000000/01001001_空城计.json
...
def run_survey(limit: int = 50):
    ...
    # 保存调研报告
    report_path = PROJECT_ROOT / "data" / "dialogue_format_survey.json"
...
def _save_dialogue_export(
    ...
    if output_dir is None:
        output_dir = PROJECT_ROOT / "data" / "db_exports"
...
def run_batch(
    ...
    # 输出 JSON 备份
    output_dir = PROJECT_ROOT / "data" / "dialogue_parsing_results"
    output_dir.mkdir(parents=True, exist_ok=True)
...
def _save_cooccurrence_export(
    ...
    Args:
        ...
        output_dir: 输出目录，默认为 data/db_exports/
...
def run_cooccurrence_batch(
    ...
    # ── 输出 1: 处理摘要日志 ──
    log_dir = PROJECT_ROOT / "data" / "cooccurrence_results"
