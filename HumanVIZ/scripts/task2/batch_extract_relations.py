...
DB_EXPORTS_DIR = DATA_DIR / "processed" / "task2" / "db_exports"
...
# 输出路径（后续步骤使用）
OUTPUT_DIR = DATA_DIR / "processed" / "task2" / "extracted_relations"
...
def main_step_3_3():
    ...
    parser = argparse.ArgumentParser(
        description="关系结果后处理（校验/去重/来源标记）",
...
示例:
  ...
  # 指定输入/输出路径
  python scripts/batch_extract_relations.py --post-process \\
      --input data/processed/extracted_relations/relations_batch_results.json.gz \\
      --output data/processed/extracted_relations/relations_post_processed.json.gz
...
示例:
  ...
  # 指定输出路径
  python scripts/batch_extract_relations.py --export --final-output data/processed/db_exports/角色关系.json
