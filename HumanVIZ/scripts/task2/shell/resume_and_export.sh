...
LOG="data/processed/task2/extracted_relations/batch_run_log.txt"
STEP35_LOG="data/processed/task2/extracted_relations/step35_export_log.txt"

echo "[$(date)] 检查当前状态..."
PROCESSED=$(python3 -c "import json; cp=json.load(open('data/processed/task2/extracted_relations/extraction_checkpoint.json')); print(cp['total_processed'])")
...
while kill -0 $BATCH_PID 2>/dev/null; do
    sleep 120
    CUR=$(python3 -c "import json; cp=json.load(open('data/processed/task2/extracted_relations/extraction_checkpoint.json')); print(cp['total_processed'])" 2>/dev/null || echo "?")
