/**
 * RoleTreeRing.tsx — 借鉴 Beautiful in English 的 Tree Ring 可视化方案
 *
 * 用同心圆环展示京剧四大行当（生旦净丑）的层次结构：
 * - 每个圆环 = 一个行当大类，带开口（Beautiful 风格）
 * - 中心粗体显示行当名（生/旦/净/丑）
...
import treeringData from "../../data/t1_treering.json";
      ...
      {activeCat && tooltipPos && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
            <span style={{ width: 10, height: 10, borderRadius: "50%", background: activeCat.color }} />
            <span style={{ fontWeight: 700, fontSize: 13, color: ROLE_TEXT_COLORS[activeCat.name] || INK_DARK, fontFamily: "'PT Serif', 'Noto Serif SC', serif" }}>
              {activeCat.name}行
            </span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 8 }}>
            {activeCat.subTypes.map((sub) => (
              <div key={sub.name} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: sub.color, flexShrink: 0 }} />
                <span style={{ width: 36, fontSize: 10, color: INK_SOFT }}>{sub.name}</span>
                <span style={{ flex: 1, height: 4, background: "rgba(0,0,0,0.05)", borderRadius: 2, overflow: "hidden" }}>
                  <span style={{ display: "block", height: "100%", width: `${(sub.count / activeCat.subTypes[0].count) * 100}%`, background: sub.color, borderRadius: 2 }} />
                </span>
                <span style={{ fontSize: 10, color: INK_SOFT, width: 24, textAlign: "right" }}>{sub.count}</span>
              </div>
            ))}
          </div>
          {(() => {
            ...
            return allChars.length > 0 ? (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                <span style={{ fontSize: 10, color: INK_SOFT }}>代表角色：</span>
                {allChars.map((c) => (
                  <span key={c} style={{ fontSize: 10, padding: "1px 6px", background: `${ROLE_TEXT_SOFT[activeCat.name] || "#b89b6d"}18`, border: `1px solid ${ROLE_TEXT_SOFT[activeCat.name] || "#b89b6d"}33`, borderRadius: 10, color: ROLE_TEXT_COLORS[activeCat.name] || INK_DARK }}>{c}</span>
                ))}
              </div>
            ) : null;
          })()}
