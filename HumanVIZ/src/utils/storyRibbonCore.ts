/**
 * storyRibbonCore.ts — 泛用性故事丝带（Story Ribbon）核心模块
 *
 * 最大程度复用现有 positions.ts / curve.ts / sceneData.ts / constants.ts 的代码，
...
import { getAllData, Scene, CharacterData, CharacterScene, SceneCharacter } from "./sceneData";
...
export interface RibbonAnalysisResult {
  ...
  /** v2 增强: 表演形式比率 (来自 structural_fingerprints) */
  performanceForm?: {
    singing: number;
    reciting: number;
    speaking: number;
...
export function analyzeStoryRibbons(input: RawStoryInput): RibbonAnalysisResult {
  ...
  const positions = getAllPositions(
    ...
    false, // evenSpacing
    "sceneConflict", // 场景冲突驱动 Y 轴，高潮场景升至最高点
    []
  );
  ...
  // Step 5: 计算叙事指标（优先使用预计算阶段）
  const metrics = computeNarrativeMetrics(data.scene_data, input.narrativePhases);
...
function computeNarrativeMetrics(scenes: Scene[], precomputedPhases?: NarrativePhase[]): NarrativeMetrics {
  ...
  let rhythmType: NarrativeMetrics["rhythmType"] = "单场聚焦型";

  // ── 短剧 (n ≤ 4) ──
  if (n <= 4) {
    if (sentimentVolatility > 0.4) {
      rhythmType = "密集高潮型";
    } else if (n === 1) {
      rhythmType = "单场聚焦型";
    } else {
  ...
  // ── 长篇 (n ≥ 10) ──
    ...
    if (conflictVariance < 0.05) {
      rhythmType = "长篇铺陈型";
    } else if (conflictVariance <= 0.1) {
      rhythmType = "波折递进型";
  ...
  // 自适应阶段检测（优先使用预计算阶段）
  const narrativePhases = precomputedPhases && precomputedPhases.length > 0
    ? precomputedPhases
    : detectNarrativePhases(scenes, conflictArc, sentimentArc, characterDensity);
...
export interface StoryFingerprint {
  ...
  /** 通用叙事结构指标 */
  conflictTrend: number;       // 冲突弧线性趋势（正=渐强，负=渐弱）
  sentimentTrend: number;      // 情感弧线性趋势
  conflictRange: number;       // 冲突波动范围
  peakPosition: number;        // 高潮位置 (0-1)
  structureType: string;       // 叙事结构类型
  arcShape: string;            // 弧线形态描述
  /** v2 增强字段 */
  multiClimaxIndices?: number[];  // 多高潮/转折点索引列表
  conflictType?: string;          // 冲突类型
  structureFramework?: {          // 经典叙事框架
    framework: string;
    confidence: number;
  };
...
export function extractFingerprint(result: RibbonAnalysisResult): StoryFingerprint {
  ...
  // ── 通用叙事结构分类（基于 conflict/sentiment 弧线特征）──
  const n = scenes.length;
  const conflictArc = narrativeMetrics.conflictArc;
  const sentimentArc = narrativeMetrics.sentimentArc;

  // 线性趋势
  const xMean = (n - 1) / 2;
  const yMean = conflictArc.reduce((a, b) => a + b, 0) / Math.max(n, 1);
  let num = 0, den = 0;
  for (let i = 0; i < n; i++) { num += (i - xMean) * (conflictArc[i] - yMean); den += (i - xMean) ** 2; }
  const conflictTrend = den !== 0 ? num / den : 0;

  const sMean = sentimentArc.reduce((a: number, b: number) => a + b, 0) / Math.max(n, 1);
  let sNum = 0;
  for (let i = 0; i < n; i++) { sNum += (i - xMean) * (sentimentArc[i] - sMean); }
  const sentimentTrend = den !== 0 ? sNum / den : 0;

  const conflictRange = Math.max(...conflictArc) - Math.min(...conflictArc);
  const peakIdx = conflictArc.indexOf(Math.max(...conflictArc));
  const peakPosition = n > 1 ? peakIdx / (n - 1) : 0;

  // 叙事结构类型分类
  let structureType = "未知";
  let arcShape = "";
  if (n <= 3) {
    structureType = "短剧型"; arcShape = "场景过少";
  } else if (conflictTrend > 0.015 && n >= 6) {
    structureType = "渐进高潮型"; arcShape = "冲突持续攀升";
  } else if (peakPosition < 0.25 && conflictTrend < -0.01) {
    structureType = "早期高潮型"; arcShape = "开篇即高潮，随后回落";
  } else if (conflictRange > 0.4 && conflictArc.filter((v: number, i: number) => i > 0 && Math.abs(v - conflictArc[i-1]) > 0.15).length >= 3) {
    structureType = "跌宕起伏型"; arcShape = "多次冲突转折";
  } else if (n >= 8 && conflictRange < 0.25) {
    structureType = "平稳铺陈型"; arcShape = "冲突平缓推进";
  } else if (sentimentArc.filter((v: number, i: number) => i > 0 && Math.abs(v - sentimentArc[i-1]) > 0.25).length >= 3) {
    structureType = "情感波浪型"; arcShape = "情绪多次起伏";
  } else {
    structureType = "渐进高潮型"; arcShape = "经典叙事弧线";
  }
  ...
  return {
    ...
    totalLines,
    sceneLengthCV,
    conflictTrend: Math.round(conflictTrend * 10000) / 10000,
    sentimentTrend: Math.round(sentimentTrend * 10000) / 10000,
    conflictRange: Math.round(conflictRange * 1000) / 1000,
    peakPosition: Math.round(peakPosition * 100) / 100,
    structureType,
    arcShape,
  };
}
